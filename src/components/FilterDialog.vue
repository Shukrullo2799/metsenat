<template>
  <div class="dialog">
    <div class="content">
      <div class="flex justify-between border-b py-7">
        <p class="tet-[#28293D] font-bold text-2xl leading-7">Filter</p>
      ...
      <div class="h-full py-7">
        <div class="mb-7">
          <p class="font-medium text-xs">{{ 'Ariza holati'.toLocaleUpperCase() }}</p>
          <select
            class="w-full border border-[#E0E7FF] rounded-md py-2 px-4 mt-2 text-sm text-[#28293D] font-medium"
            v-model="form.status"
          >
            <option value="1">Barchasi</option>
            <option value="2">Bekor qilingan</option>
            <option value="3">Qabul qilingan</option>
            <option value="4">Yuborilgan</option>
          </select>
        </div>
        <div class="mb-7">
          <p class="font-medium text-xs">{{ 'Ariza holati'.toLocaleUpperCase() }}</p>
        ...
        <div class="">
          <p class="font-medium text-xs">{{ 'Sana'.toLocaleUpperCase() }}</p>
          <select
            class="w-full border border-[#E0E7FF] rounded-md py-2 px-4 mt-2 text-sm text-[#28293D] font-medium"
            v-model="form.status"
          >
      ...
      <div class="flex justify-end border-t sm:flex-row flex-col py-7">
        <button
          @click="$emit('close')"
          class="flex items-center justify-center py-2 px-8 border rounded-md sm:mr-4 mb-4 sm:mb-0"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
          ...
          >
            <path
              d="M19.6449 10.0047C19.387 9.88994 16.972 8.8149 16.7132 8.70016C15.4988 8.15956 14.0765 8.70632 13.5366 9.91997L20.8647 13.1813C21.4053 11.9669 20.8586 10.5445 19.6449 10.0047ZM21.4161 2.07743C21.0111 1.89723 20.5806 1.94343 20.4489 2.19063L16.9403 8.8011L19.3076 9.85381L21.9798 2.87138C22.0753 2.60802 21.8212 2.25763 21.4161 2.07743Z"
              stroke="#B2B7C1"
              stroke-width="1.4"
              stroke-linecap="round"
              stroke-linejoin="round"
        ...
        <button
          @click="$emit('close')"
          class="bg-blue-600 text-white flex items-center justify-center py-2 px-8 rounded-md"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
