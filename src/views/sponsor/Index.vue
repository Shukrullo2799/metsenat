    ...
    <div class="h-full overflow-auto">
      <table class="w-full border-separate border-spacing-y-3">
        <thead class="text-[#B1B1B8]">
          <tr>
            <th>#</th>
            <th>{{ 'f.i.sh.'.toLocaleUpperCase() }}</th>
            <th>Tel.Raqami</th>
        ...
        <tbody>
          <tr
            v-for="(item, i) in useSponsorStore().getSponsors"
            :key="i"
            class="bg-white py-[22px] px-4 rounded-lg border border-[#2e5bff14]"
          >
            <td class="text-center">{{ i + 1 }}</td>
            <td class="font-medium">{{ item.full_name }}</td>
            <td class="text-center">{{ item.phone }}</td>
            <td class="text-center font-medium">
              <span class="text-[#2E384D]">{{ parseFloat(item.sum).toLocaleString('ru-RU') }}</span>
              <span class="text-[#B2B7C1]"> UZS</span>
            </td>
            <td class="text-center font-medium">
              <span class="text-[#2E384D]">{{
                parseFloat(item.spent).toLocaleString('ru-RU')
              }}</span>
              <span class="text-[#B2B7C1]"> UZS</span>
            </td>
            ...
            <td class="text-center text-[#FFA445]">{{ item.get_status_display }}</td>
            <td>
              <svg
